...
mkdir -p checkpoints-sft-bunny/$OUTPUT_DIR
...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port 13111 bunny/train/train.py \
    ...
    --version qwen2 \
    --data_path data \
    --image_folder datasets/folder \
    --vision_tower google/siglip-so400m-patch14-384 \
    --pretrain_mm_mlp_adapter checkpoints-pretrain-bunny/$PRETRAIN_DIR/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
    --image_aspect_ratio pad \
    --group_by_modality_length True \
    --bf16 True \
    --output_dir checkpoints-sft-bunny/$OUTPUT_DIR \
    --num_train_epochs 1 \
    ...
    --report_to none | tee 2>&1 checkpoints-sft-bunny/$OUTPUT_DIR/log.txt
